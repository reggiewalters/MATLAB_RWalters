...
% Updated 11/15/2019 to address issues with new SFPUC web filter
%
%%% USAGE:
...
% temporary certificate work-around
options = weboptions;
...
if ischar(StartDate)    StartDate = datenum(StartDate);     end
if ischar(EndDate)      EndDate   = datenum(EndDate);       end
...
if nDays < 119 && (floor(now) - StartDate) < 119
...
elseif nDays >= 120 || (floor(now) - StartDate) >= 119
    
    % River Case:
    cb1 = '00060';    % discharge [cfs]
    cb2 = '00065';    % stage [ft]
    furl = ['https://nwis.waterdata.usgs.gov/ca/nwis/uv?cb_' cb1 '=on&amp&cb_' ...
        cb2 '=on&format=rdb&period&begin_date=' startStr '&end_date=' endStr ...
        '&site_no=' siteNum];
    a = webread(furl, options);
    if numel(a) > 250
